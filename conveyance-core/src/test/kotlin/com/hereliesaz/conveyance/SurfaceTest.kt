...
class SurfaceTest {
    ...
    private fun element(id: String, rank: Rank) =
        DeclaredElement(ElementId(id), Employment.Working(Job.Invite, Job.Report, Job.Progress, Job.Interrupt), rank = rank)
    ...
    fun `a channel can only ever mean what it carries, because there is nowhere left to say otherwise`() {
        val element = DeclaredElement(
            ElementId("a"),
            Employment.Working(Job.Invite, Job.Report, Job.Progress, Job.Interrupt),
